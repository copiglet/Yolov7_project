...
def run_detect(input_id, pt_path = '',btr_type = '', dir_num = '', df = pd.DataFrame([""])):
    ...
    for j in range(21):
        emp.append('NaN')
    df = df
    emp[list(df.columns).index('case_name')] = str(input_id) 
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        if len(detail_js["boxs"]):
            ...
            for name in df.columns:
                if name == path.split('/')[-1].split('.p')[0]:
                    
                    emp[list(df.columns).index(name)] = class1
...
def ng_batch_detect():
    ...
    for i in glob.glob(rootdir+'/**',recursive=True):   
        if '.png' in i: 
            if 'Display_Cam' not in i:
                os.remove(i)
    
    for i in glob.glob(rootdir+'/**',recursive=True):    
        if '.png' in i:
            d = i.split('/')[-1].split(',')[0].split('-')[1]
            if d == '0':
                btr_type = 'm'
            elif d == '1':
                btr_type = 'p'
    ...
    if btr_type == 'm':
        df = pd.DataFrame(columns=['case_name', 'Display_Cam-0,GrabNum-00','Display_Cam-0,GrabNum-01','Display_Cam-0,GrabNum-02','Display_Cam-0,GrabNum-03','Display_Cam-0,GrabNum-04'
                           ,'Display_Cam-0,GrabNum-05','Display_Cam-0,GrabNum-06','Display_Cam-0,GrabNum-07','Display_Cam-0,GrabNum-08','Display_Cam-0,GrabNum-09',
                           'Display_Cam-0,GrabNum-10','Display_Cam-0,GrabNum-11','Display_Cam-0,GrabNum-12','Display_Cam-0,GrabNum-13','Display_Cam-0,GrabNum-14',
                           'Display_Cam-0,GrabNum-15','Display_Cam-0,GrabNum-16','Display_Cam-0,GrabNum-17','Display_Cam-0,GrabNum-18','Display_Cam-0,GrabNum-19'])
    elif btr_type == 'p':
        df = pd.DataFrame(columns=['case_name', 'Display_Cam-1,GrabNum-00','Display_Cam-1,GrabNum-01','Display_Cam-1,GrabNum-02','Display_Cam-1,GrabNum-03','Display_Cam-1,GrabNum-04'
                           ,'Display_Cam-1,GrabNum-05','Display_Cam-1,GrabNum-06','Display_Cam-1,GrabNum-07','Display_Cam-1,GrabNum-08','Display_Cam-1,GrabNum-09',
                           'Display_Cam-1,GrabNum-10','Display_Cam-1,GrabNum-11','Display_Cam-1,GrabNum-12','Display_Cam-1,GrabNum-13','Display_Cam-1,GrabNum-14',
                           'Display_Cam-1,GrabNum-15','Display_Cam-1,GrabNum-16','Display_Cam-1,GrabNum-17','Display_Cam-1,GrabNum-18','Display_Cam-1,GrabNum-19'])  

    dir_num = rootdir.split('/')[-1]

    if not os.path.exists(os.path.join('/home/huvio/Project/huvio/data/ai_data/2023-03-15', btr_type, dir_num)):
        os.mkdir(os.path.join('/home/huvio/Project/huvio/data/ai_data/2023-03-15', btr_type, dir_num))
    dstdir = os.path.join('/home/huvio/Project/huvio/data/ai_data/2023-03-15', btr_type, dir_num, 'output')
    ...
    for jobitem in pred_list:
        ...
        if dorun:
            code, res_js, emp = run_detect(req_id, pt_path, btr_type, dir_num, df)
            if web_debug:
    ...
    df.to_csv('/home/huvio/Project/huvio/csv/' + btr_type + '_' + dir_num + '.csv',encoding='utf-8-sig', index=False)
